...
<body>\
	<form action=\"/data\" method=\"GET\">\
      <h1>ESP32 Wi-Fi configuration </h1>\
        ...
        <strong>PSWD:</strong> <input type=\"text\" name=\"PSWD\"><br>\
        <strong>IP:</strong> <input type=\"text\" name=\"IP\">(el puerto por defecto es siempre port 50)<br><br>\
 ...
 esp_err_t data_get_handler(httpd_req_t *req)
{
    ...
    // IP:
    httpd_query_key_value(query, "IP", buff, buff_len);
    if(buff[0]=='\0'){
        printf("IP: default.\n");
    }
    else{
        nvs_set_str(my_handle,"IP",buff);
        printf("IP: %s\n",buff);
    }
